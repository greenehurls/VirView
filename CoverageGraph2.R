#library(tidyverse)
#menangle_coverage2<- readr::read_tsv("/Users/greenebp/Desktop/Sequencing2/3-24-2024-SARS-CoV-2-Spiked/20hr/fastq_pass/barcode05/sequenceresults/coverage.tsv",col_names = FALSE)
#colnames(menangle_coverage2) <- c("Virus", "Nucleotide","Coverage")
#ggplot() +geom_histogram(data=menangle_coverage2,aes(x=Nucleotide, y=after_stat(density),weight=Coverage),binwidth=1)
...
#menangle_coverage2<- readr::read_tsv("/Users/greenebp/Desktop/Sequencing2/Menangle/Targeted/fastq_pass/barcode05/sequenceresults/coverage.tsv",col_names = FALSE)
colnames(menangle_coverage2) <- c("Virus", "Nucleotide","Coverage")

genomelength <- read_tsv((commandArgs(TRUE)[2]),col_names = FALSE)
#genomelength <- read_tsv("/Users/greenebp/Desktop/Sequencing2/Menangle/Targeted/fastq_pass/barcode05/sequenceresults/genomelength.tsv",col_names = FALSE)
colnames(genomelength) <- c("genomelength")


annotated <- (commandArgs(TRUE)[6])
if(annotated==TRUE){
  geneannotations <- read_tsv((commandArgs(TRUE)[7]))
...
if(exists("annotations")) {
  h <- ggplot()+
    geom_histogram(data=menangle_coverage2,aes(x=Nucleotide, y=after_stat(count),weight=Coverage),binwidth=1)+
  ...
  p <- ggplotly(h)
  htmlwidgets::saveWidget(as_widget(p), (commandArgs(TRUE)[5]),selfcontained = F)
  #htmlwidgets::saveWidget(as_widget(p), "/Users/greenebp/Desktop/Sequencing2/Menangle/Targeted/fastq_pass/barcode05/sequenceresults/coverage.html",selfcontained = F)
  ...
  h <- ggplot()+
    ...
    theme(axis.text = element_text(colour = "black"))
    p <- ggplotly(h)
    htmlwidgets::saveWidget(as_widget(p), file=(commandArgs(TRUE)[5]),selfcontained = F)
    #htmlwidgets::saveWidget(as_widget(p), "/Users/greenebp/Desktop/Sequencing2/Menangle/Targeted/fastq_pass/barcode05/sequenceresults/coverage.html",selfcontained = F)
}
